...
#include "SGSerializerFormat.generated.h"
...
struct SGDYNAMICTEXTASSETSRUNTIME_API FSGSerializerFormat
{
	GENERATED_BODY()
public:

	FSGSerializerFormat() = default;

	explicit FSGSerializerFormat(const uint32& InTypeId);
	...
	static FSGSerializerFormat FromTypeId(uint32 InTypeId);
	 ...
	 */
	static FSGSerializerFormat FromExtension(const FString& Extension);
	...
	bool operator==(const FSGSerializerFormat& Other) const;
	bool operator!=(const FSGSerializerFormat& Other) const;
	bool operator==(const uint32& Other) const;
	bool operator!=(const uint32& Other) const;

	friend uint32 GetTypeHash(const FSGSerializerFormat& Format)
	{
	 ...
	 */
	bool ExportTextItem(FString& ValueStr, const FSGSerializerFormat& DefaultValue,
	...
	static const FSGSerializerFormat INVALID;
...
template<>
struct TStructOpsTypeTraits<FSGSerializerFormat> : public TStructOpsTypeTraitsBase2<FSGSerializerFormat>
{
	enum
	{
		WithSerializer = true,
