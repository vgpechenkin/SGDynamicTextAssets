...
/**
 * Unified property type customization for FSGDTASerializerFormat.
 *
 ...
 */
class FSGDTASerializerFormatCustomization : public IPropertyTypeCustomization
{
public:

	/** Entry in the format picker dropdown (dropdown mode). */
	struct FSGDTAFormatEntry
	{
...
private:
	...
	/** Reads the current FSGDTASerializerFormat value from the property handle. */
	void ReadCurrentValue();

	/** Writes an updated FSGDTASerializerFormat back to the property handle. */
	void WriteValue(const FSGDTASerializerFormat& NewFormat);
	...
	void OnListSelectionChanged(TSharedPtr<FSGDTAFormatEntry> SelectedEntry, ESelectInfo::Type SelectInfo);
	...
	TArray<TSharedPtr<FSGDTAFormatEntry>> AllFormatEntries;

	/** Filtered format entries shown in the dropdown list (dropdown mode). */
	TArray<TSharedPtr<FSGDTAFormatEntry>> FilteredFormatEntries;

	/** The list view widget (dropdown mode, cached for refresh). */
	TSharedPtr<SListView<TSharedPtr<FSGDTAFormatEntry>>> FormatListView = nullptr;
